...
test('wizard', async () => {
	...
	expect(await screen.findByText(/Выберите, будете ли вы извлекать поток из/)).toBeInTheDocument();
...
test('wizard: metadata', async () => {
	...
	expect(screen.queryByLabelText('Description')).toHaveValue('Live from earth. Powered by Den4iK.');
