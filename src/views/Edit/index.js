...
export default function Edit(props) {
	...
	const handleDone = async () => {
		...
		notify.Dispatch('success', 'save:ingest', i18n._(t`Канал "${$data.meta.name}" сохранен`));
	...
	const handleChannelDelete = async () => {
		...
		if (res === false) {
			...
			notify.Dispatch('warning', 'delete:ingest', i18n._(t`Канал "${$data.meta.name}" не удалось удалить`));
			return;
		}
		...
		notify.Dispatch('success', 'delete:ingest', i18n._(t`Канал "${$data.meta.name}" был удален`));
