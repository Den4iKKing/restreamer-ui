...
export default function Edit(props) {
	...
	const handleServiceDone = async () => {
		...
		notify.Dispatch('success', 'save:egress:' + _service, i18n._(t`Настройки для "${$settings.name}" сохранены`));
	...
	const handleServiceDelete = async () => {
		...
		if (res === false) {
			setSaving(false);
			notify.Dispatch('warning', 'delete:egress:' + _service, i18n._(t`Служба публикации"${$settings.name}" не удалось удалить`));
			return;
		}

		setSaving(false);

		notify.Dispatch('success', 'delete:egress:' + _service, i18n._(t`Служба публикации"${$settings.name}" был удален`));
