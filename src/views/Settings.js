...
export default function Settings(props) {
	...
	const handleSave = async () => {
		...
		if (err !== null) {
			...
			notify.Dispatch('success', 'save:settings', i18n._(t`Настройки сохранены. Все изменения вступят в силу после перезапуска приложения.`));
					...
					<TabsVerticalGrid>
						...
						<TabPanel value={$tab} index="general" className="panel">
							<Grid container spacing={2}>
								<Grid item xs={12}>
									<Grid item xs={12}>
										<PaperThumb image={settingsImage} title="Welcome to Comedoz Restreamer" height="200px" />
									</Grid>
								</Grid>
								<Grid item xs={12}>
